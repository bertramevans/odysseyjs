 ...
 *           https://sprite.smplr.com
 *
 * Hosting for generator tool sponsored by:
 *           https://smplr.com
 *
 * Website and source code:
 *           https://github.com/StupidStudio/jQuery-Sprite-Anim
 *
...
jQuery(function ($) {
  ...
  SpriteAnim.prototype.initialize = function () {
    ...
    if (isNaN(settings.frames) || settings.frames < 1)
      $.error("Frame number must be a positive digit.");
    ...
    if (settings.autoplay)
      $(this.elem).one("sheet-0-loaded", this.play.bind(this));
  };
  ...
  SpriteAnim.prototype.prepareNextSheet = function () {
    ...
    var nextSheetEl = $(this.elem)
      .children("div.sheet")
      .eq((this.getCurSheetIdx() + 1) % 2);
  ...
  SpriteAnim.prototype.showCurrentFrame = function () {
    ...
    var sheetEl = $(this.elem)
      .children("div.sheet")
      .eq(this.getCurSheetIdx() % 2);

    var newProp = {
      "background-image":
        "url(" + this.baseurl + this.getCurSheetIdx() + ".png)",
    ...
    if (newProp["background-size"] === sheetEl.css("background-size"))
      delete newProp["background-size"];
    if (
      sheetEl
        .css("background-image")
        .search(this.baseurl + this.getCurSheetIdx() + ".png") !== -1
    )
      delete newProp["background-image"];
  ...
  SpriteAnim.prototype.getSheetDimensions = function (idx) {
    ...
    var width = this.blocksize[0] * items,
      height = 0;
...
(function ($) {
  ...
  $.ImageLoader = function (src) {
    ...
    loader = $.Deferred(function (deferred) {
      ...
      events = $.map(
        ["readystatechange", "load", "abort", "error"],
        function (type) {
          return type + ns;
        }
      ).join(" ");
